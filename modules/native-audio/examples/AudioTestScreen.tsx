...
const AudioTestScreen: FC<AudioTestScreenProps> = (props: AudioTestScreenProps) => {
    ...
    const playRecording = async (): Promise<void> => {
        ...
        try {
            const result = await NativeAudio.playAudioFile(recordedFile, { volume: 1.0 });
    ...
    return (
        <View className="flex-1 p-5 bg-gray-100">
            <Text className="text-2xl font-bold mb-5 text-center">Audio Testing</Text>
            ...
            {recordedFile && (
                <View className="mb-5 p-4 bg-white rounded-lg shadow">
                    ...
                    <Text className="mb-2.5 text-xs" numberOfLines={1} ellipsizeMode="middle">
                        File: {recordedFile}
                    </Text>
                    <View className="flex-row justify-around mb-2.5">
                        <TouchableOpacity
                    ...
                    <Text>Status: {isPlaying ? 'Playing...' : 'Not playing'}</Text>
                </View>
            )}
