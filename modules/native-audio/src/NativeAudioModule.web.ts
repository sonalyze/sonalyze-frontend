...
class NativeAudioModule extends NativeModule<NativeAudioModuleEvents> {
	...
	async fileStartRecording(fileName: string): Promise<FileRecordingResult> {
		try {
			...
			const normalizedFileName = fileName.endsWith('.webm')
				? fileName
				: `${fileName}.webm`;
	...
	async fileStopRecording(): Promise<FileRecordingResult> {
		...
		try {
			const recordingPromise = new Promise<FileRecordingResult>(
				(resolve) => {
					...
					this.mediaRecorder.onstop = async () => {
						...
						console.log(
							'Recording saved. Access with path:',
							virtualPath
						);
			...
			return {
				...
				error:
					error instanceof Error
						? error.message
						: 'Unknown error stopping recording',
			};
		}
	}
